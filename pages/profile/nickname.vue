...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      // 通过事件传递昵称更新
      uni.$emit('updateNickname', { nickname: name })
...
<style scoped>
.nickname-page { position:relative; background:#000; min-height:100vh; color:#fff; padding:20px; }
.nickname-page::before { content:''; position:absolute; inset:0; background:url('/static/icons/background.svg') center/cover no-repeat; opacity:.99; pointer-events:none; z-index:0; }
...
.content-title { position:relative; z-index:1; text-align:left; color:#fff; font-size:28px; font-weight:600; margin:60px 0 80px 24px; }
.input-area { position:relative; z-index:1; margin:0 24px 80px 24px; }
.main-input { width:100%; padding:32px 24px; background:rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.2); border-radius:20px; color:#fff; font-size:22px; text-align:center; outline:none; min-height:72px; height:auto; letter-spacing:1px; line-height:1.4; word-wrap:break-word; overflow:visible; }
